...
export default function CheckOutIndex() {
  ...
  useEffect(() => { api.get(`/receptionist/check-out?page=${page}`).then((res) => setData(res.data)) }, [page])

  if (!data) return <Layout><div /></Layout>
  const { pendingCheckOuts, hotel } = data
  const list = pendingCheckOuts.data ?? []
    ...
    <Layout>
      ...
      <div className="container mx-auto px-4 py-8 relative z-10">
        <Breadcrumbs links={[
          ...
          { label: 'Check-outs', url: '#' },
        ]} />

        <div className="mb-10">
          <p className="text-xs font-medium uppercase mb-2" style={{ color: '#A0717F', letterSpacing: '0.4em' }}>{hotel.name}</p>
          <h1 className="text-3xl lg:text-5xl font-bold" style={{ color: '#EAD3CD', fontFamily: "'Georgia', serif" }}>Pending Departures</h1>
        </div>
        ...
        {list.length > 0 ? (
          <>
              ...
              {list.map((booking) => (
                <div key={booking.id} className="bg-[#383537] rounded-2xl shadow-xl p-6 border-t-[3px]" style={{ borderColor: 'rgba(160, 113, 127, 0.6)' }}>
                  ...
                  <div className="grid grid-cols-2 gap-4 mb-5">
                    <div>
                      <p className="text-[10px] uppercase font-bold tracking-widest text-[#A0717F] mb-1">Departure Date</p>
                      <p className="text-sm font-semibold text-[#EAD3CD]">{fmtDate(booking.check_out_date)}</p>
                    </div>
                    <div>
                      <p className="text-[10px] uppercase font-bold tracking-widest text-[#A0717F] mb-1">Occupied Suites</p>
                  ...
                  <div className="flex justify-between items-center pt-5 border-t border-[rgba(234,211,205,0.05)]">
                    ...
                    <Link to={`/receptionist/check-out/${booking.id}`} className="px-6 py-2.5 rounded-lg text-[10px] font-bold uppercase tracking-widest transition-all duration-300 shadow-md"
                      style={{ backgroundColor: 'transparent', border: '1px solid #A0717F', color: '#EAD3CD' }}>
                      Process Exit
                    </Link>
                  </div>
                </div>
              ...
              <div className="w-full overflow-x-auto custom-scrollbar">
                <table className="w-full text-left whitespace-nowrap">
                  <thead style={{ backgroundColor: '#2E2530', borderBottom: '1px solid rgba(160, 113, 127, 0.2)' }}>
                    <tr>
                      {['Guest Profile', 'Departure Date', 'Occupied Suites', 'Folio Balance', ''].map((h, i) => (
                    ...
                    {list.map((booking) => (
                      <tr key={booking.id} className="transition-colors duration-300">
                        ...
                        <td className="px-6 py-4 text-right">
                          <Link to={`/receptionist/check-out/${booking.id}`} className="inline-block px-6 py-2.5 rounded-md text-[10px] font-bold uppercase tracking-widest transition-all duration-300 shadow-md"
                            style={{ backgroundColor: 'transparent', border: '1px solid #A0717F', color: '#EAD3CD' }}>
                            Clear
                          </Link>
                        </td>
            ...
            <Pagination meta={pendingCheckOuts} onPage={setPage} />
          </>
        ) : (
          <div className="rounded-2xl shadow-xl p-12 text-center" style={{ backgroundColor: '#383537', borderTop: '1px solid rgba(234, 211, 205, 0.1)' }}>
            <Icon name="arrow-right" className="w-12 h-12 mx-auto mb-4" style={{ color: 'rgba(160, 113, 127, 0.4)' }} />
            <p className="text-sm font-medium uppercase tracking-widest" style={{ color: 'rgba(207, 203, 202, 0.5)' }}>No pending departures at this time.</p>
          </div>
        )}
